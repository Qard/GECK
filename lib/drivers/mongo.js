...
function Mongo(config){
  ...
  this._openDB(config.username, config.password);
}

Mongo.prototype._openDB = function(user, pass){
  var db = this.db;
  var self = this;
  db.open(function(err, db) {
    if (err) {
      throw new Error('MongoDB database could not be opened.');
    } else if (user && pass) {
      db.authenticate(user, pass, function(err, apply){
        self._setCollection(db);
      });
    } else {
      self._setCollection(db);
    }
...
Mongo.prototype._setCollection = function(db)
{
  var self = this;
  db.collection(self._collection, function(err, collection) {
    if (err) {
      db.createCollection(self._collection, function(err, collection) {
